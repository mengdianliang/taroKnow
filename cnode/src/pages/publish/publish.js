...
class Publish extends Component {
   ...
   render(){
      ...
      return (<View className='publish-topic'> 
           <Input type='text' value={isEdit?(topicInfo?topicInfo.title:''):''}  className='publish-topic-title' onInput={(e) => this.titleChange(e)} placeholder='请输入你要发布的标题' />
           <Textarea value={isEdit?(topicInfo?topicInfo.content:''):''} className='publish-topic-content' onInput={(e) => this.contentChange(e)} placeholder='请输入您要发布的内容' />
           <Picker onChange={(e) => this.changeCata(e)} mode ='selector' range={cataData} rangeKey='value'>
