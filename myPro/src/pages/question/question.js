  ...
  render () {
    ...
    return (
      <View className='question'>
            ...
            return (
              <View className='question' key={item.id}>
                ...
                <View className='question-right'>
                  <Image className='img' src={Yes} onClick={() => this.addVote(item)}/>
                  <Text>{item.vote ? item.vote : 0}</Text>
                  <Image className='img' src={No} onClick={() => this.cutVote(item)}/>
                </View>
              </View>
            )
          })
       ...
       {showQuestionModal ? 
       <AddQuestion onubmitQuestion={(obj) => this.submitQuestion(obj)} onCancelQuestion={() => this.cancelQuestion()}/> : 
       null}
       <Button className='btn-question' onClick={() => this.addQuestion()}>提问</Button>
      </View>
    )
  }
}
